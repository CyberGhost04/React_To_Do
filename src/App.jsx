    ...
    <>
      ...
      <div className={`nav h-[35vh] w-[100%] flex flex-col gap-16 align-middle justify-center ${background}`}>
        ...
        <div className="input flex justify-center align-middle">
          ...
          <button disabled={val.length<1} className='w-20 h-[43px] text-black bg-white font-bold rounded-r-[50px] border-solid border-black border-2 flex flex-col align-middle justify-around pl-2 cursor-pointer disabled:bg-[#222222] disabled:border-[#222222] disabled:text-[#222222]' onClick={AddTask}>Lets Go</button>
        </div>
      </div>
      ...
      <div className={`p-4 nav h-[42vh] w-[100%] flex align-middle justify-center ${background}`}>
        <div className="flex flex-col align-top gap-2 overflow-auto">

          {tasks.map((items) => {
            return <div key={items.id} className="text-white info flex justify-between bg-[#353839] border-[#353839] rounded-[50px] border-2 border-solid p-4 max-h-[40rem] min-w-[23rem] ">
              <p className='pl-2 overflow-y-auto overflow-x-hidden break-all max-w-[16rem] max-h-[20rem]'>{items.val}</p>
              <div className="btns flex gap-4">
                <button className='invert min-h-2 min-w-2 cursor-pointer' onClick={EditTask}><img src={Pencil} alt="" /></button>
